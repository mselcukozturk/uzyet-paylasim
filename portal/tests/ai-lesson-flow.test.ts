...
void test('ders içinde Konu ve Test sekmeleri checkpoint ve modül içeriklerini ayırır', () => {
  ...
  const makeLesson = new Function('pratikSeciliKonu', 'ARSIV_KONU', 'arsivSeciliDers', 'konuDotHtml',
    'escapeHtml', 'dersSekmesi', 'STATE', 'checkpointDersi', 'checkpointHatirlaticiListesi',
    'renderArsivDersSec', 'MODUL_TANIMLARI', 'PRATIK_TUM_MODULLER', 'pratikHavuz',
    'pratikTileStil', 'pratikEnIyiHtml', 'pratikDevamHtml', 'arsivDersModulleri',
    fn('renderPratikModul') + '\nreturn renderPratikModul;');
  const render = (tab: string) => (makeLesson(
    'Kambiyo', 'Arşiv', null, () => '', (value: string) => value, tab,
...
void test('checkpoint bitince eşlenmiş, soru içeren modülün testi açılır', () => {
  ...
  const state = { practiceBank: [{ konu: 'Kambiyo', modul: 'DT1' }] };
  ...
  assert.deepEqual(hedef('ark_n-uluslararasi'), { konu: 'Kambiyo', modul: 'DT1' });
  assert.equal(hedef('ark_n-akreditif'), null, 'soru içermeyen modüle buton verilmez');
  assert.equal(hedef('bilinmeyen'), null);
  state.practiceBank.push({ konu: 'Ürünler', modul: 'U18' });
  assert.deepEqual(hedef('ark_n-dijital'), { konu: 'Ürünler', modul: 'U18' }, 'güncel test gelene dek eski modül kullanılabilir');
  state.practiceBank.push({ konu: 'Ürünler', modul: 'UR2' });
  assert.deepEqual(hedef('ark_n-dijital'), { konu: 'Ürünler', modul: 'UR2' }, 'yeni modül gelince öncelik ona geçer');
  ...
  assert.match(finished(), /data-action="checkpoint-testi-coz" data-konu="Kambiyo" data-modul="DT1">Testi Çöz<\/button>/);
