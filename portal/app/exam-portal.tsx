...
import { ArrowLeft, ArrowRight, BookOpenCheck, Check, CircleAlert, History, LockKeyhole, Play, RotateCcw, Save, UserRound, X } from 'lucide-react';
...
function LoginView({ demo, onDemoEnter }: { demo: boolean; onDemoEnter: () => void }) {
  ...
  async function submit(event: React.SyntheticEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!result.ok) return setError(result.message ?? 'Giriş bilgileri doğrulanamadı.');
  ...
  async function requestPasswordReset() {
    ...
    if (!identifier.trim()) return setError('Önce e-posta adresini veya kullanıcı adını yaz.');
    setBusy(true);
    try {
      const result = await requestPasswordResetAction(identifier);
      if (!result.ok) return setError(result.message ?? 'Şifre bağlantısı gönderilemedi.');
      setResetMessage('Hesap eşleşiyorsa şifre oluşturma bağlantısı e-posta adresine gönderildi.');
    } catch {
      setError('Şifre bağlantısı şu anda gönderilemedi.');
    } finally {
      setBusy(false);
    }
  }
      ...
      <div className="login-layout">
        ...
        <section className="surface-card login-card" aria-labelledby="login-title">
          ...
          <p className="mt-2 text-sm leading-6 text-muted-foreground">
            Yalnız davetli hesaplar erişebilir.
          </p>
          <form className="mt-6 space-y-4" onSubmit={submit}>
            <label className="form-field" htmlFor="login-identifier">
              ...
              <div className="relative">
                ...
                <Input id="login-identifier" className="h-11 pl-10" value={identifier} onChange={(event) => setIdentifier(event.target.value)} placeholder="ad.soyad@example.com" required={!demo} autoComplete="username" />
              </div>
            </label>
            {!demo && (
              <label className="form-field" htmlFor="login-password">
                ...
                <div className="relative">
                  ...
                  <Input id="login-password" className="h-11 pl-10" type="password" value={password} onChange={(event) => setPassword(event.target.value)} required autoComplete="current-password" />
                </div>
              </label>
            )}
            <Button className="h-11 w-full" type="submit" disabled={busy}>
              {demo ? 'Yerel önizlemeye geç' : busy ? 'Giriş yapılıyor…' : 'Giriş yap'}
            </Button>
            {!demo && (
              <Button className="h-10 w-full" type="button" variant="ghost" disabled={busy} onClick={requestPasswordReset}>
                İlk şifreni oluştur veya şifreni yenile
              </Button>
            )}
          </form>
          {demo && <StatusNotice>Yerel önizleme modu: gerçek kullanıcı veya soru verisi kullanılmıyor.</StatusNotice>}
          {resetMessage && <StatusNotice tone="success">{resetMessage}</StatusNotice>}
...
export default function ExamPortal({ demoMode, initialIdentity }: { demoMode: boolean; initialIdentity: string | null }) {
  const demo = demoMode;
  const [view, setView] = useState<View>(initialIdentity ? 'dashboard' : 'login');
  ...
  const loadDashboard = useCallback(async () => {
    ...
    try {
      const data = await callExamApi({ action: 'dashboard' }, demo) as DashboardData;
  ...
  useEffect(() => {
    if (initialIdentity) queueMicrotask(() => void loadDashboard());
  }, [initialIdentity, loadDashboard]);

  const startAttempt = useCallback(async (mode: ExamMode, examCode?: string) => {
    const data = await callExamApi({ action: 'start', mode, examCode }, demo) as ActiveAttempt;
    setAttempt(data);
    setView('exam');
    return data;
  }, [demo]);
  ...
  const resume = useCallback(async (attemptId: string) => {
    setBusy(true); setError(null);
    try { setAttempt(await callExamApi({ action: 'resume', attemptId }, demo) as ActiveAttempt); setView('exam'); }
  ...
  const cancelAttempt = useCallback(async (attemptId: string) => {
    setBusy(true); setError(null);
    try { await callExamApi({ action: 'cancel', attemptId }, demo); await loadDashboard(); }
  ...
  const answer = useCallback(async (questionId: string, selectedIndex: number) => {
    ...
    try { await callExamApi({ action: 'answer', attemptId: attempt.id, questionId, selectedIndex }, demo); }
    catch (err) {
      setAttempt((current) => {
  ...
  const flagQuestion = useCallback(async (questionId: string, note: string) => {
    if (!attempt) return;
    setError(null);
    try { await callExamApi({ action: 'flag', attemptId: attempt.id, questionId, note }, demo); }
    catch (err) {
  ...
  async function pause() {
    if (!attempt) return;
    setBusy(true);
    try { await callExamApi({ action: 'pause', attemptId: attempt.id }, demo); await loadDashboard(); }
  ...
  async function finish() {
    if (!attempt) return;
    setBusy(true); setError(null);
    try { const data = await callExamApi({ action: 'finish', attemptId: attempt.id }, demo) as ExamResult; setResult(data); setAttempt(null); setView('result'); }
    catch (err) { setError(err instanceof Error ? err.message : 'Sınav tamamlanamadı.'); }
    finally { setBusy(false); }
  }

  async function signOut() {
    if (!demo) await signOutAction();
  ...
  if (view === 'login') return <LoginView demo={demo} onDemoEnter={() => { setIdentity('Yerel Önizleme'); void loadDashboard(); }} />;
